"""OpenGL and text rendering for the Voidside cone scene."""
...
class ConeRendererMixin:
    ...
    def draw_target(self):
        ...
        # Rest at camera center, with occasional short stutters that snap back.
        dot_pos = [
            self.target_position[0] + self.dot_offset[0],
            self.target_position[1] + self.dot_offset[1],
            self.target_position[2] + self.dot_offset[2]
    ...
    def draw(self, display_width, display_height):
        ...
        if self.loading_effect_active and self.loading_text_visible:
            ...
            # camera transform
            # OpenGL post-multiplies transforms: roll must be specified first so
            # it is applied last, around the camera's view axis rather than a
            # world-space axis that would knock the target off-center.
